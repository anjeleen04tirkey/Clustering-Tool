...
if selected_rows is not None and not selected_rows.empty:
    ...
    for row in selected_rows:
        if isinstance(row, dict) and "STORE" in row:
            ...
            cluster_data = knn_data[knn_data["Cluster"] == test_cluster]

            knn_filtered = NearestNeighbors(n_neighbors=5)
            knn_filtered.fit(cluster_data.drop(["STORE"], axis=1))

            distances, indices = knn_filtered.kneighbors(cluster_data[cluster_data['STORE']==store_id].drop(["STORE"], axis=1).values)
            control_stores = selected_data.iloc[indices[0]].copy()
